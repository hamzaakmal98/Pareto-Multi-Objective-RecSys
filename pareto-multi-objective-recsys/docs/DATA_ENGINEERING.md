...
Appendix: recommended minimal artifact layout

```
data/
	raw/KuaiRand-Pure/
	interim/joined_interaction_{run_id}.parquet
	interim/video_stats_snapshot_{run_id}.parquet
	processed/train.parquet
	processed/val.parquet
	processed/test_random_exposure.parquet

artifacts/
	feature_metadata/feature_metadata.yaml
	feature_metadata/category_encoder.joblib
	leakage_checks/{run_id}.log
	provenance/{run_id}_provenance.json
```

If you want, I can also:

- add an example `scripts/prepare_data.py` to generate the joined/interim artifacts, or
- create a `feature_metadata.yaml` template and encoder saving/loading utilities under `src/data`.

- `data/raw/` - original dataset files (do not edit)
- `data/interim/` - intermediate artifacts and extracted features
- `data/processed/` - final train/val/test splits and cleaned CSVs used for modeling

Include reproducible preprocessing scripts under `src/data` and `scripts/`.
